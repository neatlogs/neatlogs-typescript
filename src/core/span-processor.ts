...
export class NeatlogsSpanProcessor implements SpanProcessor {
  ...
  onEnd(span: ReadableSpan): void {
    ...
    try {
      ...
      spanData = this._normalizeFrameworkSpanName(spanData);
      spanData = this._injectCrewaiTaskTemplate(spanData);
  ...
  private _normalizeFrameworkSpanName(
    s: Record<string, any>,
  ): Record<string, any> {
  ...
  private _injectCrewaiTaskTemplate(
    s: Record<string, any>,
  ): Record<string, any> {
    const attrs: Record<string, any> = s.attributes ?? {};
