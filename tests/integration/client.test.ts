...
describe("Client", () => {
  ...
  it("routes concurrent trace calls to independent providers", async () => {
    ...
    expect(
      first.exporter.getFinishedSpans().map((span) => span.name),
    ).toContain("first-run");
    expect(
      first.exporter.getFinishedSpans().map((span) => span.name),
  ...
  it("reports a per-client timeout without flushing foreign providers", async () => {
    const entry = makeClient("stuck-flush");
    const clientFlush = vi
    ...
    expect(result.outcomes).toContainEqual(
      expect.objectContaining({
        pipeline: expect.stringContaining("stuck-flush"),
        success: false,
        timedOut: true,
      }),
    );
  ...
  it("bounds shutdown when a custom exporter never settles", async () => {
    ...
    const client = new Client({
      apiKey: "unused",
      workflowName: "stuck-shutdown",
      spanExporter: exporter,
    });
    clients.push(client);

    await expect(client.shutdown("test", 10)).resolves.toBe(false);
  ...
  it("routes one shared provider wrapper at invocation time", async () => {
    ...
    expect(
      firstSpans.some((span) => span.name === "openai.chat.completions.create"),
    ).toBe(true);
    expect(
      secondSpans.some(
  ...
  it("copies the process-scoped verification marker into its resource", async () => {
    ...
    try {
      ...
      clients.push(client);
      expect(
        client.tracerProvider.resource.attributes[
