...
describe('NeatlogsSpanProcessor', () => {
  ...
  describe('_normalizeFrameworkSpanName', () => {
    ...
    it('should not rename non-task spans', () => {
      ...
      const result = (processor as any)._normalizeFrameworkSpanName(spanData);
      expect(result.name).toBe('some-span');
    });
  });

  // ── CrewAI task template injection ────────────────────

  describe('_injectCrewaiTaskTemplate', () => {
    it('should inject template from registry', async () => {
      ...
      expect(result.attributes['neatlogs.task.user_prompt_template']).toBe(
        'Research {{topic}}',
      );
      expect(
        result.attributes['neatlogs.task.user_prompt_template_variables'],
      ).toBe('{"topic": "AI"}');
