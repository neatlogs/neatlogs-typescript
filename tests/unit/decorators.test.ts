...
describe("decorateSpan", () => {
  ...
  it("keeps an async-generator span open until all chunks are consumed", async () => {
    ...
    expect(mockSpan.addEvent).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        "neatlogs.stream.chunk.summary":
          expect.stringContaining("encoded_bytes"),
      }),
    );
  ...
  it("keeps a span open when a promise resolves to a ReadableStream", async () => {
    const wrapped = decorateSpan(
      { kind: "CHAIN" },
      async () =>
        new ReadableStream<string>({
  ...
  it("should use spanName override if provided", () => {
    const fn = () => 42;
    const wrapped = decorateSpan(
      { kind: "CHAIN", spanName: "custom-name" },
      fn,
    );
...
describe("span()", () => {
  ...
  it("should set AGENT attributes", () => {
    ...
    const wrapped = span(
      { kind: "AGENT", role: "researcher", goal: "find papers" },
      fn,
    );
    wrapped();
    expect(mockSpan.setAttribute).toHaveBeenCalledWith(
...
describe("retrieverPostprocessor", () => {
  ...
  it("should not fail on null/undefined result", () => {
    expect(() =>
      retrieverPostprocessor(spanObj as any, null, {}),
    ).not.toThrow();
    expect(() =>
      retrieverPostprocessor(spanObj as any, undefined, {}),
    ).not.toThrow();
  });
